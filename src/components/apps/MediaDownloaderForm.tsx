 ...
 */
export default function MediaDownloaderForm() {
  /**
  ...
  onMount(() => {
    ...
    socket.on('download_complete', async (data) => {
      ...
      const fileUrl = await getBlobFileUrl(data.filePath);
      setFilePath(fileUrl);
    });
  ...
  return (
    <div class="media-downloader-wrapper space-y-4">
      <input
        type="text"
      ...
      {filePath() && (
        <>
          ...
          format().startsWith('webm') ||
          format().startsWith('flv') ||
          format().startsWith('.webm') ? (
            <VideoJSPlayer
              options={{
                sources: [
